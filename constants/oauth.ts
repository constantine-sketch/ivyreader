...
const bundleId = "space.manus.ivyreader.t20260130164246";
 ...
 * - Native: uses deep link scheme (manually constructed to avoid Expo dev scheme)
 */
export const getRedirectUri = () => {
  if (ReactNative.Platform.OS === "web") {
    return `${getApiBaseUrl()}/api/oauth/callback`;
  } else {
    // Manually construct the redirect URI to ensure we use the custom scheme
    // instead of Expo's dev scheme (exp://)
 ...
 *
 * On native platforms (iOS/Android), use expo-web-browser to open an in-app browser
 * so users stay within the app during authentication.
 *
